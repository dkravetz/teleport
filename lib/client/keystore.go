...
func (fs *FSLocalKeyStore) AddKey(key *Key) error {
	if err := key.KeyIndex.CheckAllSet(); err != nil {
		return trace.Wrap(err)
	}

	writeBytes := func(bytes []byte, path ...string) error {
		fp := fs.inSessionKeyDir(key.ProxyHost, filepath.Join(path...))
	...
	// Store common key data.
	if err := writeBytes(key.Priv, key.Username); err != nil {
		return trace.Wrap(err)
	}
	if err := writeBytes(key.Pub, key.Username+fileExtPub); err != nil {
		return trace.Wrap(err)
	}
	if err := writeBytes(key.TLSCert, key.Username+fileExtTLSCert); err != nil {
		return trace.Wrap(err)
	}

	// Store per-cluster key data.
	if err := writeBytes(key.Cert, key.Username+sshDirSuffix, key.ClusterName+fileExtSSHCert); err != nil {
		return trace.Wrap(err)
	}
	...
	for kubeCluster, cert := range key.KubeTLSCerts {
		...
		if err := writeBytes(cert, key.Username+kubeDirSuffix, key.ClusterName, kubeCluster+fileExtTLSCert); err != nil {
			return trace.Wrap(err)
		}
	}
	for db, cert := range key.DBTLSCerts {
		dbCluster := filepath.Clean(db)
		if err := writeBytes(cert, key.Username+dbDirSuffix, key.ClusterName, dbCluster+fileExtTLSCert); err != nil {
			return trace.Wrap(err)
		}
	}
	for app, cert := range key.AppTLSCerts {
		appCluster := filepath.Clean(app)
		if err := writeBytes(cert, key.Username+appDirSuffix, key.ClusterName, appCluster+fileExtTLSCert); err != nil {
			return trace.Wrap(err)
		}
	}
...
func (fs *FSLocalKeyStore) DeleteKeys() error {
	err := os.RemoveAll(fs.inSessionKeyDir())
	if err != nil {
		return trace.ConvertSystemError(err)
	}
	return nil
...
func (fs *FSLocalKeyStore) GetKey(idx KeyIndex, opts ...CertOption) (*Key, error) {
	if len(opts) > 0 {
		if err := idx.CheckAllSet(); err != nil {
	...
	for _, o := range opts {
		certPath := fs.inSessionKeyDir(o.relativeCertPath(idx))
		certData, err := fs.loadCertData(certPath)
		if err != nil {
		...
		if err := o.updateKey(key, certData); err != nil {
			fs.log.Error(err)
			return nil, trace.Wrap(err)
		}
	}
...
func (fs *FSLocalKeyStore) loadCertData(certPath string) (interface{}, error) {
...
type CertOption interface {
	...
	// updateKey adds the cert data to the key and performs related checks.
	updateKey(key *Key, certData interface{}) error
...
func (o WithSSHCerts) updateKey(key *Key, certData interface{}) error {
	cert, ok := certData.([]byte)
	if !ok {
		return trace.BadParameter("unexpected type %T, expected []byte", certData)
	}
	key.Cert = cert
...
func (o WithKubeCerts) updateKey(key *Key, certData interface{}) error {
	certs, ok := certData.(map[string][]byte)
	if !ok {
		return trace.BadParameter("unexpected type %T, expected map[string][]byte", certData)
	}
	key.KubeTLSCerts = certs
	return nil
}
...
func (o WithDBCerts) updateKey(key *Key, certData interface{}) error {
	certs, ok := certData.(map[string][]byte)
	if !ok {
		return trace.BadParameter("unexpected type %T, expected map[string][]byte", certData)
	}
	key.DBTLSCerts = certs
	return nil
}
...
func (o WithAppCerts) updateKey(key *Key, certData interface{}) error {
	certs, ok := certData.(map[string][]byte)
	if !ok {
		return trace.BadParameter("unexpected type %T, expected map[string][]byte", certData)
	}
	key.AppTLSCerts = certs
	return nil
}
