...
// KeyIndex helps to identity a key in the store.
...
// CheckAllSet verifies the KeyIndex is fully specified.
func (idx KeyIndex) CheckAllSet() error {
	missingField := "key index field %s is not set"
	if idx.ProxyHost == "" {
		return trace.BadParameter(fmt.Sprintf(missingField, "ProxyHost"))
	}
	if idx.Username == "" {
		return trace.BadParameter(fmt.Sprintf(missingField, "Username"))
	}
	if idx.ClusterName == "" {
		return trace.BadParameter(fmt.Sprintf(missingField, "ClusterName"))
	}
	return nil
}
